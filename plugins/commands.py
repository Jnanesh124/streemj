...
async def start(client, message):
            ...
            try:
                ...
                await client.send_cached_media(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [                                                                                               
                                InlineKeyboardMarkup('🚀 OTT RELEASED MOVIES JOIN 🚀', url=f'https://t.me/+D7L-rX9lKA43MGRl')
                            ]
                        ]
                    )
                )
    ...
    if data.startswith("sendfiles"):
        ...
        k = await client.send_message(chat_id=message.from_user.id,text=f"<b>Get All Files in a Single Click!!!\n\n📂 ʟɪɴᴋ ➠ : {g}\n\n🎟 Note: This message is deleted in 5 mins to avoid copyrights. Save the link to Somewhere else</b>", reply_markup=InlineKeyboardMarkup(
                [
                    [
        ...
        await k.edit("<b>Your message is successfully deleted!!! if you want again type here 👇</b>")
        return
        ...
        for x in filesarr:
            await x.delete()
        await k.edit_text("<b>Your All Files/Videos is successfully deleted!!! if u want again type here 👇</b>")
        return    
        
    elif data.startswith("files"):
        user = message.from_user.id
        if temp.SHORT.get(user)==None:
            await message.reply_text(text="<b>Please type again here 👇 in this bot</b>")
        else:
            chat_id = temp.SHORT.get(user)
        ...
        if settings['is_shortlink'] and user not in PREMIUM_USER:
            ...
            await asyncio.sleep(1200)
            await k.edit("<b>Your message is successfully deleted!!! Please type again here 👇 in this bot</b>")
            return
    user = message.from_user.id
    files_ = await get_file_details(file_id)           
    if not files_:
        ...
        try:
            ...
            title = '@Rockers_ott_movie_link_bot @kannadavideosnewbot  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@Rockers_ott_movie_link_bot @kannadavideosnewbot  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@Rockers_ott_movie_link_bot @kannadavideosnewbot  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
...
async def shortlink(bot, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete :(\n\nGive me a shortener website link and api along with the command !\n\nFormat: <code>/shortlink shareus.io 1VykTPGi38gGZtJufYaOXWroTOh2</code></b>")
